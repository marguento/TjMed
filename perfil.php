<div class="space60"></div>

  <div class="container">
    <div class="row">
 		<div class="col-md-12">
          <h1>Nombre_de_Negocio_Ejemplo</h1>
        </div>
    </div>
 </div>

		<div class="space20"></div>
 <div class="container">  
    <div class="blog-container"> 
      <div class="row"> 
      
        <div class="col-md-8 blog-content">
          
          <!-- Blog Post -->
          <div class="space30"></div>
          <img src="imss2.jpg" alt="" width="734" height="293">
          <div class="space25"></div>
          <h2>Galleria</h2>
          <div class="space10"></div>
          
          <div class="post-info-container">
            <div class="row">
              <div class="col-md-8">
                <div class="post-info">
                  <span class="post-data">
                    <i class="fa fa-user"></i> <a href="#">Usuario</a>
                  </span>
                  <span class="post-data">  
                    <i class="fa fa-tag"></i> <span class="tags"><a href="#">Imagen</a> | <a href="#">Portrait</a></span>
                  </span>
                  <span class="post-data">  
                    <i class="fa fa-comment"></i> 12 Comentarios
                  </span> 
                </div>
              </div>
              <div class="col-md-4">
                <div class="social-2 f-right">
                  <a href="#"><i class="fa fa-apple"></i></a>
                  <a href="#"><i class="fa fa-facebook"></i></a>
                  <a href="#"><i class="fa fa-foursquare"></i></a>
                  <a href="#"><i class="fa fa-google-plus"></i></a>
                  <a href="#"><i class="fa fa-pinterest"></i></a>
                  <a href="#"><i class="fa fa-youtube"></i></a>
                </div>  
              </div>
            </div>
          </div>
  
          <div class="space30"></div>
          <p>       
          Vis ad timeam vivendo necessitatibus, viris nonumy abhorreant id vel, an munere eruditi praesent qui. Usu noster legendos cu. Mei omnium graecis an. Te nam graeci nostrud dissentiet. Usu quem appellantur id, ut debet accommodare his. Vel te dicit putant facilis, ius ad torquatos moderatius.
          </p><p>
          Ut simul appellantur cum, ut reque legimus ocurreret ius. Nec ne aliquam scriptorem, mundi primis recteque id sed, pri laudem meliore te. Eam nihil officiis conceptam ei, pro ex eros velit invenire. Zril petentium an est, amet putant eum eu, usu iudico possim voluptatum ad. An sea vidisse alienum.
          Nam modo agam eius ea, duo paulo propriae ei. Te case illud his, duo invenire vituperata liberavisse ex. Vix vero persius ut. Te invenire instructior mea, vis inani primis facilisis at, minimum molestie complectitur sit ea. Idque corpora pertinacia usu cu, vel eu possim detraxit gloriatur.
          </p>
          <div class="space20"></div>
          
          <blockquote>       
          Mummy abhorreant id vel, an munere eruditi praesent qui. Usu noster legendos cu. Mei omnium graecis an. Te nam graeci nostrud dissentiet. Usu quem appellantur id, ut debet accommodare his. Vel te dicit putant facilis, ius ad torquatos moderatius.
          Lantur cum, ut reque leit invenire. Zril petentium an est, amet putant eum eu, usu iudico possim voluptatum ad. An sea vidisse alienum.
          </blockquote>
                                    
          <div class="space35"></div>
          <a href="negocios.html" class="btn"><i class="fa fa-book"></i>Más Negocios</a>
          <div class="space40"></div>
          <!-- Blog Post End -->

        </div>
        
        <div class="col-md-4 blog-right-sidebar">
        
          <div class="">
            <div class="space25"></div>
            <h4>Ubicación</h4>
            
			<iframe width="350" height="350" frameborder="0" scrolling="no" marginheight="0" marginwidth="0" src="http://maps.google.com/maps?client=safari&q=phoenix+az&oe=UTF-8&ie=UTF8&hq=&hnear=Phoenix,+Maricopa,+Arizona&gl=us&ll=33.448377,-112.074037&spn=0.040679,0.07699&z=14&output=embed"></iframe><br><small><a href="http://maps.google.com/maps?client=safari&q=phoenix+az&oe=UTF-8&ie=UTF8&hq=&hnear=Phoenix,+Maricopa,+Arizona&gl=us&ll=33.448377,-112.074037&spn=0.040679,0.07699&z=14&source=embed" style="color:#0000FF;text-align:left">Alargar Mapa</a></small>

            <div class="space40"></div>
            
            <h4>Cliente Satisfecho</h4>
            <div class="client-says">
              <div class="client-text">   
                “Clariteniam, quis nostrud exercitation ullamco laboris. Aute irure dolor in repreh wypas candy canes toffee. Co adipisicing elit, sed do eiusmod tempor incididunt ut laboenderit in voluptate velit esse cillum dudium lectorum. Mirum est notare quam liquam at erat in purus aliquet mollis. Fusce ele velit nibeh.”
              </div>  
              <div class="client-name">
                <i class="fa fa-quote-right"></i><strong>Manuel Soto</strong>, Apple.
              </div>  
            </div> 
            
            <div class="space40"></div>
           
          </div>  
          
        </div>
        
      </div> 

<div class="col-md-12">
	<div class="space40"></div>
        <h3>Comentarios (4)</h3>
        <div class="space40"></div>
        
        <!-- Blog Comment -->
        <div class="blog-comment">
          <div class="user-image"><i class="fa fa-user"></i></div> 
          <div class="comment-data">
            <h4>Pancho Perez</h4>
            <p>18 Febrero 2014, 9:43 pm<a href="#" class="reply-link">Responder</a></p>
            <p>
              Ique torquatos. Vitae inciderint qui cu, cu quo choro denique lobortis. Sit euripidis aliquando in, quot commodo in pro, eos id equidem legimus pertinacia. Ei mei dico enim, vivendum principes mei in. Cu primis maiorum usu. Eam et erant laudem tacimates, ad pro vitae legimus. Cum at quas habeo, erat primis delectus sit no, id zril quaestio mel. Tale nullam expetendis eam ne, his ne eros minimum sadipscing, quas noster at vis. Mel discere iudicabit te, error pericula ei sit. Mea eu soleat regione, liber postula.
            </p>
            <div class="divider"></div>           
          </div> 
        </div>
        <div class="space30"></div> 
        <!-- Blog Comment End -->
        
        <!-- Blog Comment -->
        <div class="blog-comment">
          <div class="user-image"><i class="fa fa-user"></i></div> 
          <div class="comment-data">
            <h4>Mario Martinez</h4>
            <p>18 Febrero 2014, 10:02 pm<a href="#" class="reply-link">Responder</a></p>
            <p>
              Cucumber quo choro denique lobortis. Sit euripidis aliquando in, quot commodo in pro, eos id equidem legimus pertinacia. Ei mei dico enim, vivendum principes mei in. Cu primis maiorum usu. Eam et erant laudem tacimates, ad pro vitae legimus. Cum at quas habeo, erat primis delectus sit no, id zril quaestio mel. Tale nullam expetendis eam ne, his ne eros minimum sadipscing, quas noster at vis. Mel discere iudicabit te, error pericula ei sit. Mea eu soleat regione, liber postula.
            </p>
            <div class="divider"></div>
          </div> 
        </div>
        <div class="space30"></div> 
        <!-- Blog Comment End -->
  
        <!-- Blog Comment -->
        <div class="blog-comment">
          <div class="user-image"><i class="fa fa-user"></i></div> 
          <div class="comment-data">
            <h4>Ana Trejo</h4>
            <p>19 Febrero 2014, 5:12 am<a href="#" class="reply-link">Responder</a></p>
            <p>
              Riderint qui cu, cu quo choro denique lobortis. Sit euripidis aliquando in, quot commodo in pro, eos id equidem legimus pertinacia. Ei mei dico enim, vivendum principes mei in. Cu primis maiorum usu. Eam et erant laudem tacimates, ad pro vitae legimus. Cum at quas habeo, erat primis delectus sit no, id zril quaestio mel. Tale nullam expetendis eam ne, his ne eros minimum sadipscing, quas noster at vis. Mel discere iudicabit te, error pericula ei sit. Mea eu soleat regione, liber postula.
            </p>
            <div class="divider"></div>
            <div class="space20"></div> 
            
              <!-- Blog Comment Reply -->
              <div class="blog-comment reply">
                <div class="user-image"><i class="fa fa-user"></i></div> 
                <div class="comment-data">
                  <h4>Wanda Dabalos</h4>
                  <p>21 Febrero 2014, 11:29 am<a href="#" class="reply-link">Responder</a></p>
                  <p>
                    Vitae inciderint qui cu, cu quo choro denique lobortis. Sit euripidis aliquando in, quot commodo in pro, eos id equidem legimus pertinacia. Ei mei dico enim, vivendum principes mei in. Cu primis maiorum usu. Eam et erant laudem tacimates, ad pro vitae legimus. Cum at quas habeo, erat primis delectus sit no, id zril quaestio mel. Tale nullam expetendis eam ne, his ne eros minimum sadipscing, quas noster at vis. Mel discere iudicabit te, error pericula ei sit. Mea eu soleat regione, liber postula.
                  </p>
                    <div class="divider-2"></div>
                    <div class="space20"></div> 

                      <!-- Blog Comment Reply -->
                      <div class="blog-comment reply">
                        <div class="user-image"><i class="fa fa-user"></i></div> 
                        <div class="comment-data">
                          <h4>Israel Villalobos</h4>
                          <p>23 Febrero 2014, 2:18 pm<a href="#" class="reply-link">Responder</a></p>
                          <p>
                            Choro denique lobortis. Sit euripidis aliquando in, quot commodo in pro, eos id equidem legimus pertinacia. Ei mei dico enim, vivendum principes mei in. Cu primis maiorum usu. Eam et erant laudem tacimates, ad pro vitae legimus. Cum at quas habeo, erat primis delectus sit no, id zril quaestio mel. Tale nullam expetendis eam ne, his ne eros minimum sadipscing, quas noster at vis. Mel discere iudicabit te, error pericula ei sit. Mea eu soleat regione, liber postula.
                          </p>
                          <div class="divider-2"></div>
                          <div class="space20"></div> 
                        </div> 
                      </div>
                      <div class="space30"></div> 
                      <!-- Blog Comment Reply End -->

                  </div>     
                </div>
                <div class="space30"></div> 
                <!-- Blog Comment Reply End -->

          </div>          
        </div>
        <!-- Blog Comment End -->

      </div>

    </div>  
  </div>

<div class="promo-box" style="padding-top: 35px; padding-bottom: 35px;">
    <div class="container">  
      <div class="row">
        <div class="col-md-8 promo-text">
          <h4>Subir una foto de este Negocio</h4>
        </div>
        <div class="col-md-4 right">
          <button class="btn btn-white">Elegir Foto</button>
        </div>
      </div>    
    </div>   
  </div>

</div>
</div>  
		<div class="space60"></div>
		<!-- Content End -->